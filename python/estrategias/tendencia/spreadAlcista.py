...
class SpreadAlcista(object):
    ...
    def __init__(self, pe1, p1, pe2, p2, tipo, numContratos, numContratos2):
        if (pe1 < pe2):
            ...
            self.eje_y = []
            self.eje_0 = []
            self.nombre = "Spread Alcista"
            if self.tipo == 0:
                self.rutaImagen, self.eje_x, self.eje_y, self.eje_0 = spreadAlcistaCall(self.precioEjercicio, self.precioEjercicio2, self.prima,self.prima2, self.numContratos, self.numContratos2)
            if  self.tipo == 1:
                self.rutaImagen, self.eje_x, self.eje_y, self.eje_0 = spreadAlcistaPut(self.precioEjercicio, self.precioEjercicio2, self.prima,self.prima2, self.numContratos, self.numContratos2)
        else:
            raise ValueError("El precio de ejercicio de la compra tiene que ser menor que el de la venta")

def spreadAlcistaCall(precioEjercicio, precioEjercicio2, prima, prima2, numContratos, numContratos2):
...
def spreadAlcistaPut(precioEjercicio, precioEjercicio2, prima, prima2, numContratos, numContratos2):
    multiplo = max(prima, prima2)
    precioSubyacenteInferior = int(precioEjercicio - (multiplo * 2))
    precioSubyacenteSuperior = int(precioEjercicio + (multiplo * 2))
