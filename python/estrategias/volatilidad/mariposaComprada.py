...
class MariposaComprada(object):
    ...
    def __init__(self, pe1, p1, pe2, p2, pe3, p3, pe4, p4):
        if (pe1 < pe2 and pe1 < pe3 and pe1 < pe4 and pe2 == pe3 and pe3 < pe4 and p2 == p3):
            ...
            self.eje_y = []
            self.eje_0 = []
            self.nombre = "Mariposa Comprada"

            self.rutaImagen, self.eje_x, self.eje_y, self.eje_0 = mariposaComprada(self.precioEjercicio, self.precioEjercicio2, self.precioEjercicio3, self.precioEjercicio4, self.prima,self.prima2, self.prima3, self.prima4)
        else:
            raise ValueError("No se complen las condiciones de la mariposa comprada")

def mariposaComprada(precioEjercicio, precioEjercicio2, precioEjercicio3, precioEjercicio4, prima, prima2,prima3, prima4):
    multiplo = max(prima, prima2, prima3, prima4)
    ...
    for i in range(precioSubyacenteInferior, precioSubyacenteSuperior + 1):
        ...
        if i <= precioEjercicio:
            ...
            eje_y.append(i - precioEjercicio - prima)

        if i <= precioEjercicio2:
            eje_y1.append(prima2)
        else:
            eje_y1.append((i - precioEjercicio2 - prima2)*-1)

        if i <= precioEjercicio3:
            eje_y2.append(prima3)
        else:
            eje_y2.append((i - precioEjercicio3 - prima3)*-1)

        if i <= precioEjercicio4:
            eje_y3.append(-prima4)
        else:
            eje_y3.append(i - precioEjercicio4 - prima4)
