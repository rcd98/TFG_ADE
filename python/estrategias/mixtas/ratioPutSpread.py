...
class RatioPutSpread(object):
    ...
    def __init__(self, pe1, p1, pe2, p2, pe3, p3):
        if (pe1 > pe2  and pe2 == pe3 and p2 == p3):
            ...
            self.eje_y = []
            self.eje_0 = []
            self.nombre = "Ratio Put Spread"

            self.rutaImagen, self.eje_x, self.eje_y, self.eje_0 = ratioPutSpread(self.precioEjercicio, self.precioEjercicio2, self.precioEjercicio3, self.prima,self.prima2, self.prima3)
        else:
            raise ValueError("No se complen las condiciones del ratio put spread")

def ratioPutSpread(precioEjercicio, precioEjercicio2, precioEjercicio3, prima, prima2,prima3):
    ...
    for i in range(precioSubyacenteInferior, precioSubyacenteSuperior + 1):
        ...
        if i >= precioEjercicio:
            ...
            eje_y.append((precioEjercicio - i - prima))

        if precioEjercicio2 - i + precioEjercicio2 <= precioEjercicio2:
            eje_y1.append(prima2)
        else:
            eje_y1.append((precioEjercicio2 -i - prima2)*-1)

        if precioEjercicio3 - i + precioEjercicio3 <= precioEjercicio3:
            eje_y2.append(prima3)
        else:
            eje_y2.append((precioEjercicio3 -i - prima3)*-1)
